...
- A proposer (bot key, or the owner implicitly) can propose arbitrary Merkle roots. The contract enforces proof membership, solvency, a per-round share cap, a proposal rate limit and a guardian pause -- not fair allocation or holder eligibility. The proposer cannot directly call the keeper-only payout method. The keeper now independently recalculates historical eligibility and payouts; checking only a copied proposer journal was demonstrated to be insufficient. The owner can still appoint keepers and propose roots. The 24-hour timelock and the guardian only help if somebody is alerted and acts. Exclusions, weighting and thresholds remain off-chain policy.
