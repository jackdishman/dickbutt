...
> **Historical snapshot — 13 September 2026.** For the current candidate, findings and test results, read [AUDIT_REPORT.md](../AUDIT_REPORT.md) and [the current handoff](HANDOFF.md). The selected rewards pool now uses unstaked **basic volatile ERC-20 LP tokens**, with six-hour reward periods and zero extra review delay. The NFT setup, 24-hour delay, test counts, live-task wording and pending soak below describe the earlier candidate and observations; they are retained as historical evidence, not current deployment instructions or proof of completed monitoring.
...
**Historical staged Aerodrome NFT handoff (superseded):** the earlier DICKBUTT/SPCXc LP NFT design allowed transfer to its harvester after deployment. The harvester's manager and NFT ID were immutable constructor arguments, so the exact NFT ID had to be known before deploying that harvester. If the NFT would be minted later, that harvester would be deployed only after minting established the ID; other components could be prepared earlier. This historical sequence used `safeTransferFrom`, followed by `holdsPosition()` and fee-collection checks. It is not the selected basic volatile ERC-20 LP handoff; see [the current setup sequence](../AERODROME-SETUP.md). These production actions were not performed by the historical test runs.
