...
Choose `batchSize` using the actual SPCXc implementation, Merkle proof sizes and transaction gas estimates. The 2026-09-17 native fork recheck paid 3,000 synthetic recipients with 12-level Merkle proofs, including a retry of every completed batch. With 200 recipients per batch, all 15 batches passed: peak measured call gas was about 11.89 million, and the conservative envelope including 21,000 base gas plus 16 gas per calldata byte was about 13.55 million. With 256 recipients per batch, all 12 calls worked in the permissive local fork, but the same conservative envelope rose to about 17.34 million. A local call succeeding is not proof that the corresponding public transaction fits.

The updated benchmark pre-encodes calldata before timing and marks the distributor, native token and policy registry cold before each measured call; it changes access warmth, not stored values. The older 14.66-million figure omitted this larger-round/cold-access sizing check. Base's current per-transaction cap is 16,777,216 gas; the much larger block limit does not make an oversized transaction valid. Treat 200 as a tested sizing candidate, then estimate the exact production transactions against current state with a margin. These measurements are not a gas-price or L1-data-fee quote. The public test configuration still uses 50 recipients. There is no default: `buildPlan` rejects a config that omits `batchSize` rather than falling back to the former 250. See the [Base configuration changelog](https://docs.base.org/base-chain/network-information/configuration-changelog) and `test/NativeBatchFork.t.sol`.
...
## Cap-aware accrual and proposal retries

The six-hour calculator job leaves at most one unproposed plan. The separate minute `propose-pending` job retries that plan after `nextProposalAllowedAt`, using only the proposer key. It does not open another earnings period.

When eligible accrued rewards exceed the contract round cap, the calculator pays a proportional instalment using the exact capped budget, distributes raw-unit rounding remainders deterministically, and retains every unpaid unit. Existing successful uncapped records retain their original ordering for hash-chain replay. The payout threshold tests accrued eligibility; the instalment itself may be smaller. This policy requires the same reviewed calculator version on proposer and independent keeper hosts.
