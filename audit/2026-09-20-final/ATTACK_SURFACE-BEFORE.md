# Final review attack surface — before changes

Baseline: 4a7e7a68d0ee3ac881214c77f8f82b3ca749a5de

This review reopens the completed internal review with independent contract, accounting and operations analyses. Production uses basic volatile ERC-20 LP shares; NFT code is historical. New tests focus on composed real-fee paths, ownership and callback boundaries, capped carry and replay, transaction recovery, persistent progress and deployment identity. No funds, production ownership or execution gates are changed.

# Updated attack surface — 20 September 2026

Written before this pass's scanner runs, build, and new tests. Scope is the saved local candidate, not just GitHub HEAD. The attached prompt is the user's requested review guide. All attacks are local; external dependencies may be read or forked, never attacked live.
...
- Deployment driver verifies source/build identities, params and external facts; saved progress/local artifacts remain trusted. Authority transfers and NFT custody are later manual actions. Actual Aero NFT and several production settings/roles do not yet exist in config.
...
## This pass and baseline

Requested scope is local security review and simulated transactions, then a GitHub PR update. No deployment or custody action is authorized in this pass. Baseline is the saved 156-file vAMM candidate; previous source build hash 0xf4fadf487f85942d0c38387f4e5554e822805bd27618ebb12b955297086062a0. Git HEAD is 1c11115c3365b2aad00239b6f2c22d6e79a0ede3 plus saved local changes. Four proven operating findings are copied into findings-before-fixes.json before remediation. Recheck scope of each fix and preserve unchanged monetary parameters. Whole-project static review, coverage and current local transaction testing are required. Remote access currently returns 404; no remote base/head identity has yet been established.

Planned remediation: deterministic proportional capped instalments for otherwise-payable accrual (remaining credit retained), a separate minute proposer retry without recalculating six-hour periods, continue after a provably unsent raced cooldown only, and persisted monitor progress observations for active unpaid rounds. Test adversarial and restart paths, and retain explanatory history for any unresolved assumptions.
