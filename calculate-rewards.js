...
import {validateCalculatorConfigShape} from './calculator/config.js';
...
export async function main(){
 ...
 if(configPath){
  ...
  validateCalculatorConfigShape(config);
 ...
 else console.log(stringify({block:record.block,curve:config.curve,bootstrap:record.bootstrap,roundId:record.roundId,root:record.root,total:record.plan?.total??'0',batches:record.plan?.batches.length??0,superseded:(record.prunedRounds??Object.values(record.state?.plans??{})).filter(p=>p.superseded).map(p=>p.roundId),journal:path.join(dir,'periods')}));
