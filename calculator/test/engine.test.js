...
test('all reads pinned, huge round id preserved, recovery and local collision cannot double accrue',async()=>{const f=fixture();const r=await runCalculator(f);assert.equal(r.plan.roundId,'9007199254740993');assert.equal(r.plan.total,'100');const j=new Journal(f.dir);assert.equal(j.rebuild().lastProcessedBlock,10);fs.unlinkSync(path.join(f.dir,'state.json'));f.advance();await assert.rejects(runCalculator(f),/unproposed/);assert.equal(j.rebuild().lastProcessedBlock,10);assert.ok(f.calls.length>=5);});
